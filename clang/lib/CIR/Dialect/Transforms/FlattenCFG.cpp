...
public:
  ...
  void buildCatchers(cir::TryOp tryOp, mlir::PatternRewriter &rewriter,
    ...
    else if (auto tryBodyUnreachable =
                 dyn_cast<cir::UnreachableOp>(afterBody->getTerminator())) {
      auto blk = tryBodyUnreachable->getBlock();
      rewriter.setInsertionPointToEnd(blk);
      auto newBlock = blk->splitBlock(tryBodyUnreachable);
      // auto unreachableBlock = rewriter.createBlock(tryBodyUnreachable->getParentRegion());
      // rewriter.setInsertionPointToStart(unreachableBlock);
      // rewriter.create<cir::UnreachableOp>(tryOp.getLoc());
      rewriter.create<cir::BrOp>(tryOp.getLoc(), newBlock);
    }
